#!/bin/bash
# web-search.sh — Dispatch web search to Tier 3 via Gemini grounding
...
T_START=$(date +%s%3N)
bash "$OPS_LOG" "Dispatching to Tier 3: web search — ${QUERY:0:100}"
...
T_SSH=$(date +%s%3N)
RESULT=$(ssh tier3 "GEMINI_API_KEY='$GEMINI_KEY' SEARCH_QUERY='$QUERY' NODE_PATH=/usr/lib/node_modules node /home/ubuntu/scripts/web_search.js" 2>&1) || true
