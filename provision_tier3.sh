#!/bin/bash
# provision_tier3.sh — OpenClaw Tier 3 (Fully-Exposed) provisioning script
# Runs ON the Tier 3 VM as root or via sudo
set -euo pipefail

TIER1_IP="174.129.11.27"

echo "=== [1/8] SSH Hardening ==="
# Disable password auth (should already be off, but enforce)
sudo sed -i 's/^#*PasswordAuthentication.*/PasswordAuthentication no/' /etc/ssh/sshd_config
sudo sed -i 's/^#*ChallengeResponseAuthentication.*/ChallengeResponseAuthentication no/' /etc/ssh/sshd_config
sudo sed -i 's/^#*UsePAM.*/UsePAM no/' /etc/ssh/sshd_config
# Ensure only key auth
...
echo "=== [4/8] Install Gemini CLI ==="
# Gemini CLI from Google (the agent tool for Tier 3)
sudo npm install -g @anthropic-ai/claude-code 2>/dev/null && echo "Claude Code installed (fallback agent)" || true
sudo npm install -g @google/generative-ai 2>/dev/null && echo "Google Generative AI SDK installed" || true
# Try the Gemini CLI package
sudo npm install -g gemini 2>/dev/null && echo "Gemini package installed" || true
echo "Gemini tooling installation complete. Verify with 'gemini --version' or check npm global list."
echo "Installed global npm packages:"
npm list -g --depth=0 2>/dev/null || true

echo "=== [5/8] Prepare Gemini configuration ==="
# Create config directory
sudo mkdir -p /home/ubuntu/.config/gemini
...
# Create environment file template (no secrets — user fills in)
...
# Gemini API Key (injected by Tier 1)
# GEMINI_API_KEY=

# Email IMAP credentials (read-only app passwords, injected by Tier 1)
...
echo "=== [6/8] Prepare email config templates ==="
sudo mkdir -p /home/ubuntu/email-config
cat > /home/ubuntu/email-config/README.txt << 'EMAILREADME'
EMAIL CONFIGURATION FOR TIER 3
===============================

Tier 3 needs READ-ONLY IMAP access to:
1. Microsoft Exchange (Outlook/Office365)
2. Gmail
...
For Exchange:
  - Create an App Password in Microsoft 365 admin
  - Ensure the app password has IMAP read-only scope (no send)
  - IMAP server: outlook.office365.com:993 (SSL/TLS)

For Gmail:
  - Enable IMAP in Gmail settings
  - Create an App Password (requires 2FA enabled)
  - The app password should be for "Mail" only
  - IMAP server: imap.gmail.com:993 (SSL/TLS)

IMPORTANT:
  - Do NOT store credentials on this VM
  - Tier 1 injects credentials at runtime via SSH environment variables
  - Credentials are cleared on session end and on nightly rebuild
EMAILREADME
chown -R ubuntu:ubuntu /home/ubuntu/email-config

echo "=== [7/8] Disable unnecessary services ==="
# Disable SSM agent if present
...
# Disable cups-browsed if present
...
echo "=== [8/8] Create results directory for Tier 1 to pull ==="
mkdir -p /home/ubuntu/results
chown ubuntu:ubuntu /home/ubuntu/results

echo ""
echo "=== Provisioning complete ==="
echo "Tier 3 (OpenClaw Fully-Exposed) is configured."
echo ""
echo "Remaining manual steps:"
echo "  1. Gemini API key will be injected by Tier 1 at runtime"
echo "  2. Email app passwords will be injected by Tier 1 at runtime"
echo "  3. Tier 1 SSH key needs to be added to authorized_keys"
